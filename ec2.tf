...
locals {
  ...
  # ========================================================================
  # âœ… ADD THIS SECTION - Configure and Restart CloudWatch Agent
  # ========================================================================
  echo ""
  ...
  sudo sed -i "s/csye6225-webapp-dev/csye6225-webapp-${var.environment}/g" "$CLOUDWATCH_CONFIG"

  echo "Updated CloudWatch config for environment: ${var.environment}"
  ...
  if systemctl is-active --quiet amazon-cloudwatch-agent; then
    echo "âœ" CloudWatch Agent is running"
    sudo systemctl status amazon-cloudwatch-agent.service --no-pager
  else
    echo "âœ— CloudWatch Agent failed to start"
    sudo systemctl status amazon-cloudwatch-agent.service --no-pager
    exit 1
  fi
  # ========================================================================
  ...
  if systemctl is-active --quiet webapp.service; then
    ...
    echo "âš  Application not running yet"
  fi
